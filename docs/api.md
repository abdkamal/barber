...
- `POST /sync/events` بجسم `{events: [...]}` (حتى 200) ← مصفوفة `[{eventId, result: applied|duplicate|rejected, reason?}]` بترتيب الإرسال. (مراجعة ق40 2026-09-24) ما وقع أثناء فترة إيقاف سابقة للحساب يُطبّق ويُعلَّم للمراجعة (`GET /manager/recovered-events`، `kind: during_suspension`)؛ وإن أُوقف الحساب أثناء معالجة الدفعة تتوقف بـ`401 UNAUTHENTICATED` ويبقى الباقي على الجهاز؛ رفع متزامن للحدث نفسه يعود `duplicate`.
...
| `absent_today` | `{reason}` |
| `absent_cancelled` | — (المرحلة 11) |

- **المرحلة 11 — التراجع عن «لن أعمل اليوم»:** `absent_cancelled` يحذف غياب اليوم التشغيلي للحساب نفسه داخل قفل اليوم، فيعود اليوم إلى حالته الطبيعية (`day_state`) ويُستأنف الحجز، ويُعاد حساب الطابور بالسبب `barber_returned` («عاد الحلاق للعمل اليوم»). الحجوزات التي نقلها المدير أثناء الغياب **لا تعود** تلقائيًا. **المدير** يتراجع دائمًا (عن غيابه هو بهذا الحدث، وعن غياب أي حلاق بـ`DELETE /manager/absences/{id}`)؛ **الحلاق** فقط عن غياب سجّله بنفسه، وإلا `rejected` بالسبب `ABSENCE_SET_BY_MANAGER`. لا غياب اليوم ← `applied` بالسبب `NOT_ABSENT` (تكرار آمن). سجل تدقيق `staff.absence_cancelled`، وتنبيه `barber_absence_cancelled` للمديرين حين يتراجع **حلاق** (لا حين يتراجع مدير عن يومه، ولا للأحداث المستردة ق40).
- `day.absence` في `GET /staff/today` (المرحلة 11): `{reason, recordedBy: self|manager, canUndo}` أو `null` إن لم يكن غائبًا — `canUndo` صحيح للمدير دائمًا وللحلاق إن سجّله بنفسه.
- **المرحلة 11:** `break_started` في يوم غياب (`absent_today`) ← `rejected` بالسبب `BARBER_ABSENT`.

## المدير
| المورد | الطلبات |
|---|---|
| ملف الصالون | `GET /manager/profile`، `PUT /manager/profile` (تحديث جزئي: `{name?, about?, address?, location?: {lat, lng} \| null, phone?, whatsapp?, socialLinks?[{platform, url}]}`) — روابط التواصل `https://` فقط (L3)؛ `whatsapp` رقم دولي كامل بأي رمز دولة (`+` أو `00` ثم 8–15 رقمًا، يُخزَّن `+…`)، وإلا `400 VALIDATION_FAILED` بالتفصيل `invalid_international_phone` (ق41) |
...
| الإجازات (غائب اليوم) | `GET /manager/absences`؛ `POST /manager/absences` بجسم `{staffId, workDate, reason?}`؛ `DELETE /manager/absences/{id}` ← `{ok: true}` — داخل قفل اليوم، مع `day_state` لأجهزة الحلاق (I6). الحذف (إلغاء الغياب — المرحلة 11) يعيد حساب الطابور بالسبب `barber_returned` |
...
- `POST /manager/staff/{id}/recover-events` بجسم `POST /sync/events` نفسه `{events: [...]}` (حتى 200 حدث؛ الأكثر يُقسَّم) ← `200 {staffId, suspendedAt, results: [{eventId, result: applied|duplicate|rejected_after_suspension|rejected_uncertain_time|rejected_invalid, reason?}] (بترتيب الإرسال), summary: {applied, duplicate, rejectedAfterSuspension, rejectedUncertainTime, rejectedInvalid}}`. **مراجعة 2026-09-24:** الحدث بـ`approximate: true` لا يُطبّق أبدًا في هذا المسار ← `rejected_uncertain_time` (`reason: "UNCERTAIN_TIME"`)، ويُحفظ عنصرًا في قائمة المراجعة بحالة `not_applied_uncertain_time` مع `payloadSummary`. يرفعه المدير من جهاز الحساب الموقوف: الأحداث تُنسب لذلك الحساب وتمر بالتحقق المسبق وآلة الحالات نفسيهما، ومرة واحدة لكل `id` (إعادة الرفع ← `duplicate`، أو النتيجة نفسها لما رُفض). **يُطبّق فقط ما `occurredAt` (المصحّح) فيه قبل `suspendedAt` تمامًا** وليس أقدم منه بأكثر من 48 ساعة؛ ما عدا ذلك `rejected_after_suspension` (`reason: "AFTER_SUSPENSION"`) أو `rejected_invalid` (`EVENT_TOO_OLD`، `NOT_YOUR_BOOKING`، `UNKNOWN_EVENT_TYPE`، …) ولا يُطبّق. المطبَّق يُعلَّم `flagged` و`recovered_by_staff_id` (في `device_events` و`booking_events`) ويُدرج في قائمة المراجعة؛ ويُكتب تدقيق `staff.events_recovered` لكل رفع (حتى إن فشل في منتصفه: `complete: false` بالنتائج المعروفة)، وتدقيق `staff.event_recovered` لكل حدث داخل معاملته. حالة الحساب تُعاد قراءتها داخل معاملة كل حدث: إن أُعيد تفعيله أو أُوقف من جديد أثناء الرفع يتوقف الرفع بـ`409 ACCOUNT_NOT_SUSPENDED` دون تسجيل الباقي (يبقى على الجهاز). رفعان متزامنان للحدث نفسه ← أحدهما `applied` والآخر `duplicate`. لا تنبيهات للمدير من هذا المسار. حد المعدل كدفعات المزامنة (لكل مدير). الأخطاء: `403` لغير المدير، `404 NOT_FOUND` (معرّف غير موجود في هذا الصالون)، `409 ACCOUNT_NOT_SUSPENDED`، `400 VALIDATION_FAILED`.
- `GET /manager/recovered-events?status=pending|all` (الافتراضي `pending`؛ الأحدث أولًا، حتى 500) ← `[{id, kind: recovered|during_suspension, status: applied|not_applied_uncertain_time, staffId, staffName, eventId, type, bookingId, customerName, occurredAt, approximate, payloadSummary, suspendedAt, reactivatedAt, reason, recoveredBy: {id, name} | null, recoveredAt, reviewedAt, reviewedBy: {id, name} | null}]`. `kind: during_suspension` (مراجعة 2026-09-24): حدث وصل بالمزامنة العادية `POST /sync/events` بعد إعادة التفعيل ووقته المصحّح داخل فترة إيقاف (أو تقريبي وفي الحساب فترة إيقاف انتهت بعده) — طُبّق وعُلّم `flagged` ويُعرض هنا (`recoveredBy: null`، `reactivatedAt` نهاية الفترة). `payloadSummary` حقول معروفة فقط (`amount`، `serviceIds`، `steps`، `decision`، `reason`، `kind`، `durationMin`).
...
## التنبيهات (FCM)
`{type, bookingId, title, body}` — الأنواع: `booking_confirmed`، `called`، `eta_changed`، `postponed`، `no_show`، `cancelled_closing`، `transferred`، `overrun` (للحلاق)، `account_pending`، `barber_not_connected`، `barber_absent`، `barber_absence_cancelled` (المرحلة 11)، `sync_conflict`، `base_duration_suspect` (للمدير). قيم `data` نصية. النصوص كما في التصميم §8.
