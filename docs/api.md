...
| `POST /bookings/{id}/seen` | `{eta}` — ما عرضه التطبيق فعلًا؛ مرجع التنبيه الإلزامي (ق5). `204` دائمًا، لكن **يُسجَّل فقط إن كان ضمن 5 دقائق من إسقاط السيرفر الحالي** وإلا يُتجاهل (H1)، ولا يدخل أبدًا في إعفاء ق23 (يُحكم فيه بما سجّله السيرفر وحده) |
...
- **يوم العمل التشغيلي (C1):** `GET /staff/today` و`POST /heartbeat` و`GET /manager/queues` والمؤقتات تعمل على الدوام الجاري، أو — بعد الإغلاق — على آخر يوم ما زال فيه حجز نشط (`in_service` / `waiting` / `called`، مثل «خدمة بعد الإغلاق» ق24) حتى يبدأ اليوم التالي (فتح حجزه)، ثم على اليوم التالي. لا يُقبل بعد الإغلاق حجز جديد ولا زبون حاضر. عند بدء اليوم التالي يُغلق السيرفر اليوم السابق: `waiting`/`called` ← `cancelled` بسبب `day_closed` (`cancel_reason = "day_closed"`، تنبيه `cancelled_closing` مع `data.reason = "day_closed"`)، و`in_service` يبقى ليُنهيه الحلاق مع `needsReview: true` وتعارض للمدير (`unfinished_at_day_close`)؛ ولا يظهر أيٌّ منها في `GET /bookings/current` ولا يُحتسب ضمن الحجوزات النشطة.
...
| الطاقم | `GET/POST /manager/staff`، `PUT /manager/staff/{id}`، `POST /manager/staff/{id}/reset-code`. عنصر القائمة يضيف `is_owner`. المالك لا يُخفَّض ولا يوقَف (`409 OWNER_PROTECTED`)، ويبقى للصالون مدير نشط واحد على الأقل (`409 LAST_MANAGER`) — L5 |
